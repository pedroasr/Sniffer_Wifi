...
const parser = serialport.pipe(new ReadlineParser({delimiter: limit,
encoding:'hex'})) 
...
parser.on('data',function(buff){
    ...
    if(chain.length==46*2){
        ...
        if(chain[0] == 'f' && chain[1] == 'a' && chain[2] == 'f' && chain[3] == 'a'){
            
            //ID
            dato.id = parseInt(chain[4]+chain[5],16)

            //MAC
            dato.mac = chain[6]+chain[7]+":"+chain[8]+chain[9]+":"+chain[10]+chain[11]+":"+chain[12]+chain[13]+":"+chain[14]+chain[15]+":"+chain[16]+chain[17]
